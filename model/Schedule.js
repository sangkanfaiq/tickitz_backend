...
module.exports = {
  ...
  add: (req, res) => { // add done
    return new Promise((resolve, reject) => {
      ...
      db.query(
        `INSERT INTO schedule(movieID, price, cinemaID, locationID, dateStart, dateEnd, time) VALUES('${movieID}','${price}','${cinemaID}','${locationID}','${dateStart}','${dateEnd}','${time}')`,
        (err, results) => {
          if (err) {
            console.log(err);
  ...
  update: (req, res) => { // update done
    return new Promise((resolve, reject) => {
      ...
      db.query(`SELECT * FROM schedule where scheduleID=${scheduleID}`, (err, results) => {
        ...
        db.query(
          `UPDATE schedule SET movieID='${movieID}', price='${price}', cinemaID='${cinemaID}', locationID='${locationID}', dateStart='${dateStart}', dateEnd='${dateEnd}', time='${time}' WHERE id='${id}'`,
          (err, results) => {
            if (err) {
              console.log(err);
              reject({ message: "Something wrong" });
